...
public class MiskatonicMysteriesAPI {
	...
	public static Affiliation getApparentAffiliationFromEquipment(@Nullable ItemStack exclude, LivingEntity entity) {
		var trinkets =
				TrinketsApi.getTrinketComponent(entity).map(component -> component.getEquipped(stack -> !stack.equals(exclude) && stack.getItem() instanceof Affiliated));
	...
	public static boolean grantBlessing(PlayerEntity player, Affiliation affiliation) {
		...
		if (player instanceof ServerPlayerEntity && ascendant.getBlessings().size() < Constants.DataTrackers.MAX_BLESSINGS) {
	...
	public static void setBiomeMask(ServerWorld world, BlockPos pos, Biome biome) {
		...
		PlayerLookup.tracking(world, pos).forEach(serverPlayerEntity -> SyncBiomeMaskPacket.send(serverPlayerEntity,
				chunk));
	}
