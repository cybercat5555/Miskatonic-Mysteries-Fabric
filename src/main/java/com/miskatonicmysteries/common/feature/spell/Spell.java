	...
	public boolean cast(LivingEntity caster) {
		...
		if (!caster.world.isClient) {
			...
			if (spellCaster.isPresent() && !(caster instanceof PlayerEntity && ((PlayerEntity) caster).isCreative())) {
				int burnout = MathHelper.clamp(spellCaster.get().getSpellCooldown() + Math
					.round(medium.getCooldownModifier(caster) * effect.getCooldownBase(intensity)) + cooldownMod, 0, 999);
				spellCaster.get().setSpellCooldown(burnout);
			}
			Sanity.of(caster).ifPresent(
				sanity -> sanity.setSanity(sanity.getSanity() - effect.calculateSanityPenalty(caster.getRandom(), intensity), false));
