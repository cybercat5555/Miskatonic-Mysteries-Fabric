...
public class ElderianWallBlock extends WallBlock {
	...
	public ElderianWallBlock(Settings settings) {
		super(settings);
		this.setDefaultState((((((((this.stateManager.getDefaultState()).with(VARIANTS, 0)).with(UP, true)).with(NORTH_SHAPE, WallShape.NONE))
			.with(EAST_SHAPE, WallShape.NONE)).with(SOUTH_SHAPE, WallShape.NONE)).with(WEST_SHAPE, WallShape.NONE)).with(WATERLOGGED, false));
	}
