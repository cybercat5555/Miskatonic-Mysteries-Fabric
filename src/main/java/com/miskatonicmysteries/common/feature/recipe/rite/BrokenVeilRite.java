...
public class BrokenVeilRite extends SpawnerTrapRite {
    public BrokenVeilRite() {
        super(new Identifier(Constants.MOD_ID, "broken_veil"), null,
                ...
                Ingredient.ofItems(MMObjects.IRIDESCENT_PEARL), Ingredient.ofItems(Items.PHANTOM_MEMBRANE), Ingredient.ofItems(Items.ENDER_PEARL), Ingredient.ofItems(MMObjects.INFESTED_WHEAT));
    }
    ...
    public void tick(OctagramBlockEntity octagram) {
        ...
        if (octagram.tickCount > 100 && octagram.tickCount % 100 == 10) {
            octagram.getWorld().getEntitiesByClass(LivingEntity.class, new Box(octagram.getSummoningPos().add(-5, -3, -5), octagram.getSummoningPos().add(5, 3, 5)), Objects::nonNull)
