...
public abstract class SummoningRite<T extends Entity> extends AscensionLockedRite {
	...
	public void onFinished(OctagramBlockEntity octagram) {
		if (octagram.getWorld() instanceof ServerWorld) {
			...
			T entity = summon
				.create(world, nbt, null, octagram.getOriginalCaster(), octagram.getPos(), SpawnReason.MOB_SUMMONED, true, false);
	...
	public void renderRite(OctagramBlockEntity entity, float tickDelta, MatrixStack matrixStack, VertexConsumerProvider vertexConsumers,
		...
		if (entity.tickCount > 0) {
			...
			model.render(matrixStack, ResourceHandler.TOTAL_DARK.getSprite()
				.getTextureSpecificVertexConsumer(vertexConsumers.getBuffer(RenderHelper.getTransparency())), 0, overlay, 0, 0, 0, alpha);
