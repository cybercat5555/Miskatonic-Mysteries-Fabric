...
public class CommandDrivenRite extends Rite implements DataSerializable<Rite> {
	...
	public CommandDrivenRite(Identifier id, Affiliation affiliation, Identifier tickFunctionId, Identifier finishFunctionId, int tickCount,
		float investigatorChance, Ingredient... ingredients) {
	...
	public void tick(OctagramBlockEntity octagram) {
		if (!octagram.getWorld().isClient) {
			ServerCommandSource source = octagram.getWorld().getServer().getCommandSource().withPosition(Vec3d.ofCenter(octagram.getPos()))
				.withEntity(octagram.getOriginalCaster());
	...
	public static class Serializer extends DataReader<Rite> {
		...
		public Rite readFromJson(Identifier id, JsonObject json) {
			return new CommandDrivenRite(id, JsonHelper.hasElement(json, "affiliation") ? MMRegistries.AFFILIATIONS
				.get(new Identifier(JsonHelper.getString(json, "affiliation"))) : null, new Identifier(JsonHelper.getString(json, "tick")),
				new Identifier(JsonHelper.getString(json, "result")), JsonHelper.getInt(json, "duration", 100),
				JsonHelper.getFloat(json, "investigatorChance", 0.25F), readIngredients(JsonHelper.getArray(json, "ingredients")));
		}
