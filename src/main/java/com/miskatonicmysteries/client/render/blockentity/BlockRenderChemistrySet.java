...
public class BlockRenderChemistrySet extends BlockEntityRenderer<BlockEntityChemistrySet> {
    public BlockRenderChemistrySet(BlockEntityRenderDispatcher dispatcher) {
        super(dispatcher);
    }

    @Override
    public void render(BlockEntityChemistrySet entity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
        ...
        //determine amount, equally distribute them that way so it's always centered
        //ghost-render the three potential items, starting with the center (ghost rendering as in with decrease transparency)
        matrices.pop();
    }
