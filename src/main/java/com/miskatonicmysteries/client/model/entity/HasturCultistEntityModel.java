...
/**
 * cultist_Hastur - cybercat5555 Created using Tabula 8.0.0
 */
...
public class HasturCultistEntityModel extends BipedEntityModel<HasturCultistEntity> {
	...
	public void animateModel(HasturCultistEntity livingEntity, float f, float g, float h) {
		rightArmPose = livingEntity.getMainHandStack().isEmpty() ? ArmPose.EMPTY
																 :
					   livingEntity.isBlocking() && livingEntity.getMainHandStack().getItem().equals(Items.SHIELD) ? ArmPose.BLOCK : ArmPose.ITEM;
		leftArmPose = livingEntity.getOffHandStack().isEmpty() ? ArmPose.EMPTY
															   :
					  livingEntity.isBlocking() && livingEntity.getOffHandStack().getItem().equals(Items.SHIELD) ? ArmPose.BLOCK : ArmPose.ITEM;
	...
	@Override
	public void setAngles(HasturCultistEntity livingEntity, float limbAngle, float limbDistance, float animationProgress, float headYaw,
						  float headPitch) {
